...
class ImageToPDFConverter {
  ...
  _attachMobileListeners() {
    // Buka drawer: tombol Pengaturan di header
    this.settingsBtn?.addEventListener('click', () => this._openDrawer());

    // Tutup drawer: tombol × atau ketuk overlay
    this.panelClose?.addEventListener('click',    () => this._closeDrawer());
    this.drawerOverlay?.addEventListener('click', () => this._closeDrawer());

    // Tombol Tambah & Ekspor di bar bawah mobile
    this.mobileAddBtn?.addEventListener('click',    () => this.fileInput.click());
    this.mobileExportBtn?.addEventListener('click', () => this.createPDF());

    // Geser panel ke kiri untuk menutup (sentuh)
    if (this.panelEl) {
      ...
      this.panelEl.addEventListener('touchend', e => {
        ...
        if (e.changedTouches[0].clientX - touchStartX < -60) this._closeDrawer();
        touchStartX = null;
      }, { passive: true });
    }
  }
  ...
  handleFiles(fileList) {
    ...
    if (files.length === 0) {
      this._showToast('Hanya file gambar yang diterima.', 'warning');
      return;
    }
    ...
    Promise.all(promises).then(() => {
      ...
      this._renderThumbs();
      if (added > 0) {
        this._showToast(`${added} gambar berhasil ditambahkan.`, 'success');
      } else {
        this._showToast('Gambar sudah ada sebelumnya.', 'warning');
      }
  ...
  _createThumb(img, index) {
    ...
    div.innerHTML = `
      ...
      <div class="thumb-actions">
        <button class="thumb-action" data-action="preview" title="Pratinjau">
          <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round">
            <path d="M1 10s3.5-7 9-7 9 7 9 7-3.5 7-9 7-9-7-9-7z"/>
            <circle cx="10" cy="10" r="3"/>
          </svg>
        </button>
        <button class="thumb-action" data-action="rotate" title="Putar 90°">
          <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
            <path d="M16 8A6 6 0 1 0 9.5 14.5"/>
            <path d="M16 4v4h-4"/>
          </svg>
        </button>
        <button class="thumb-action del" data-action="remove" title="Hapus">
          <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round">
  ...
  _rotateImage(index) {
    ...
    this._showToast(`Gambar ${index + 1} diputar.`);
  }
  ...
  _removeImage(index) {
    ...
    this._showToast(
      'Gambar dihapus.',
      'default',
      [{ label: 'Urungkan', action: () => this._undoRemove() }]
    );
  }
  ...
  _undoRemove() {
    ...
    this._applySortOrder();
    this._renderThumbs();
    this._showToast('Gambar dipulihkan.', 'success');
  }

  removeAllImages() {
    if (this.images.length === 0) return;
    if (!confirm(`Hapus semua ${this.images.length} gambar?`)) return;
    this.undoStack = [];
    this.images = [];
    this._renderThumbs();
    this._showToast('Semua gambar dihapus.', 'success');
  }
  ...
  _updateStatus(totalSize) {
    ...
    if (this.mobileExportBtn) this.mobileExportBtn.disabled = n === 0;
    this.statusText.textContent  = n > 0
      ? `${n} gambar siap — ekspor jika sudah selesai.`
      : 'Tambahkan gambar untuk memulai';
  ...
  async createPDF() {
    ...
    this._showProcessingOverlay('Mempersiapkan…');
    ...
    try {
      ...
      this._showToast('Gagal membuat PDF: ' + err.message, 'error');
      this.statusText.textContent = 'Gagal — coba lagi.';
    } finally {
  ...
  async _processAllImages(doc, cfg) {
    ...
    for (let i = 0; i < total; i++) {
      ...
      const msg = `Halaman ${i + 1} dari ${total}`;
  ...
  _savePDF(doc) {
    ...
    this._showToast('PDF berhasil diekspor!', 'success');
    this.statusText.textContent = 'Selesai — PDF tersimpan.';
  }
